...
export default function PDFSigner() {
  // Signature States
  const [clickPos, setClickPos] = useState(null); // { x, y } in PDF percentage or px
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [signatureData, setSignatureData] = useState(null); // Data URL of rendered signature

  // Signature Placement & Transform State
  ...
  // 2. Setup Fullscreen Drawing Canvas
  useEffect(() => {
  ...
  // 3. Save Signature & Place at Click Coordinates
  const applySignature = () => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    // Trim whitespace around signature or export direct PNG
    const dataUrl = canvas.toDataURL('image/png');
    setSignatureData(dataUrl);

    // Center the signature over the initial click coordinate
    const defaultW = 180;
    const defaultH = 90;
    setSigTransform({
    ...
    >
      ...
      <div ref={pdfRef} style={styles.pdfPage} onClick={handlePdfClick}>
        <div style={styles.pdfContent}>
          ...
          <p>
            Please click anywhere on this page to sign. Once applied, drag the
            signature to reposition or use the bottom-right handle to scale.
          </p>
        ...
        {/* PLACED SIGNATURE WITH RESIZE / DRAG CONTROLS */}
        {signatureData && (
          <div
            style={{
      ...
      {/* FULLSCREEN SIGNATURE DRAWING MODAL */}
      {isModalOpen && (
